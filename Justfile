...
push-image $image="aurora" $tag="latest" $flavor="main" $ghcr="0" $image_registry="" $temp_push="0" $temp_push_tag="0":
    ...
    if [[ {{ ghcr }} == "1" ]]; then
        ...
        PODMAN=/home/linuxbrew/.linuxbrew/bin/podman
    fi

    if [[ "{{ ghcr }}" == "1" && -z "${image_registry}" && "${temp_push}" == "0" ]]; then
      for ${tags} in ${alias_tags}; do
    ...
    elif [[ "{{ ghcr }}" == "1" && -z "${image_registry}" && "${temp_push}" == "1" ]]; then
      # Workaround bugs caused by pushing production tags and then signing
      ${PUSH_CMD} ${image_name}:${tag} ${image_registry}/${image_name}:${temp_push_tag}
    elif [[ "{{ ghcr }}" == "0" && -z "${image_registry}" ]]; then
